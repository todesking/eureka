...
class BookListParser(HTMLParser):
    ...
    def handle_endtag(self, tag):
        if tag == "td" and self._in_td:
            ...
            if self._current_href and "ユリイカ" in text:
                parts = text.split("\u3000", 1)
                title = parts[0].strip()
...
def fetch_year(year):
    url = f"https://www.seidosha.co.jp/book/index.php?year={year}&cat_id=10"
...
def main():
    ...
    with open("eureka.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["書名", "特集タイトル", "URL"])
        writer.writerows(all_books)

    print(f"\nTotal: {len(all_books)} rows written to eureka.csv")
